...
try:
    ...
    for i in range(start_index, end_index):
        ...
        # IMRTP creation - CLICK ONCE for first iteration, TWICE for subsequent iterations
        if completed_iterations == 1:
            # First iteration - click once
            print("First iteration - clicking IMRTP creation once")
            try:
                click_img_timeout('imrtp_creation.png')
            except TimeoutError:
                click_img_timeout('imrtp_creation1.png')
        else:
            # Subsequent iterations - find once, click twice on the same spot
            print(f"Iteration {completed_iterations} - finding IMRTP creation once, clicking twice")
            try:
                # Find the image and get its position
                pt = wait_center_timeout('imrtp_creation.png')
                # Click twice on the same position
                pg.moveTo(pt.x, pt.y, duration=0.12)
                pg.click()
                time.sleep(1)  # Small delay between clicks
                pg.click()     # Second click on same position
            except TimeoutError:
                # Fallback to imrtp_creation1.png
                pt = wait_center_timeout('imrtp_creation1.png')
                pg.moveTo(pt.x, pt.y, duration=0.12)
                pg.click()
                time.sleep(1)  # Small delay between clicks
                pg.click()     # Second click on same position
